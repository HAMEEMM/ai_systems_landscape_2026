...
%%
%% DOUBLE-BLIND SUBMISSION — AUTHOR DETAILS REMOVED
%% Engineering Applications of Artificial Intelligence (EAAI)
%% Special Issue: VSI: EAAI_Agentic AI
%%
%% Engineering Applications of Artificial Intelligence (EAAI) formatting:
%%   - Uses Elsevier elsarticle document class
%%   - Review (double-spaced) layout for submission
%%   - Targeting VSI: EAAI_Agentic AI special issue
%%
...
%% See separate Title Page file for full author information.
...
The paper makes four contributions: (i)~a formal four-phase agent loop unifying existing patterns; (ii)~an 8-layer technology stack mapping 15+ frameworks; (iii)~three empirical studies validating the framework; and (iv)~a failure-mode taxonomy with EU AI Act governance mapping. This work extends a companion unified AI systems taxonomy \citep{anon2026taxonomy}.
...
The PPAS cycle generalises existing patterns, but is architecturally distinct from each. ReAct~\citep{yao2023react} collapses Plan and Act into a single interleaved step and lacks explicit self-correction. Reflexion~\citep{shinn2023reflexion} adds self-correction but does not separate planning from acting. The BDI model~\citep{rao1995bdi} separates belief updating from intention formation but lacks tool use and the iterative correction loop central to LLM agents.

OODA (Observe--Orient--Decide--Act; \citealt{boyd1987ooda}) warrants closer comparison because it too has four phases and a cyclical structure. PPAS differs from OODA in four architecturally consequential ways. First, \emph{Plan} and OODA's \emph{Orient} have distinct semantics: Orient updates a mental model in adversarial, time-critical contexts (originally fighter-pilot engagements with sub-second cycles), whereas PPAS's Plan generates goal-conditioned multi-step strategies over an explicit memory state $\mathcal{S}$, potentially issuing multi-step sub-task decompositions before committing to an action. Second, PPAS's \emph{Self-Correct} phase is not appended decoration: it is a formal feedback operator $\text{correct}(\mathcal{S}_{t-1}, a_t, o_t, g)$ that may trigger re-planning, memory consolidation, or human escalation based on observed action outcomes---none of which exist in OODA's linear decide-act sequence, which was designed for reactive tactical decisions, not iterative goal-directed computation. Third, the PPAS action space is structurally bounded by the tool set $\mathcal{T}$ in the agent tuple $\mathcal{A} = \langle \mathcal{M}, \mathcal{T}, \mathcal{S}, \pi \rangle$; OODA has no analogous concept, because the framework was designed for embodied decision-making, not for computational agents that must discover and invoke heterogeneous APIs at runtime. Fourth, OODA offers no formal memory specification---Orient implies experiential knowledge but provides no operational definition---whereas PPAS's state space $\mathcal{S}$ is an explicit, managed component enabling principled memory persistence across sessions (Section~\ref{sec:memory}). In aggregate, PPAS is not a renaming of OODA but a re-grounding of agent-loop design for the LLM era, where planning is generative and goal-conditioned, self-correction is a formal computational operator, tools are first-class architectural citizens, and memory is explicitly managed state. Table~\ref{tab:loop-comparison} presents a systematic comparison.
...
\subsection{Framework Validation: Layer Separability}\label{sec:validation}

Taken together, the three studies provide empirical validation of the 8-layer stack's core architectural claim: that the layers are \emph{separable}---independently variable without restructuring the remainder of the system. Study~2 holds five layers constant (L1: Claude 3.5 Sonnet; L2: LangGraph; L3: identical vector store; L4: Tavily + E2B + file I/O; L8: none) while varying only L6 (Planning \& Reflection design pattern), isolating a 17.7 percentage-point accuracy range on HotpotQA (61.2\% ReAct to 78.9\% HITL + Reflection). If the layer boundaries were merely taxonomic labels rather than genuine architectural seams, this controlled isolation of a single layer's effect would not be operationally achievable. Study~3 independently validates L5 (Inter-Agent Protocols) by holding all other layers constant across three communication configurations (MCP, A2A, direct), yielding a 24 percentage-point task-success range (68\%--76\%) and measurable delegation-accuracy differences attributable solely to protocol choice at L5. Study~1 corroborates at the ecosystem scale: the 39.3 percentage-point SWE-Bench gap between HITL~+ Reflection agents (72.5\%) and ReAct agents (33.2\%) across independent organisations using comparable foundation models confirms that L6 choices have first-order outcome effects independent of L1. Collectively, these three studies demonstrate that the 8-layer stack is not a descriptive taxonomy but an engineering decomposition whose layer boundaries correspond to independently measurable, causally isolable performance dimensions.
...
All datasets, benchmark results, analysis scripts, and reproduction notebooks are publicly available at \url{[URL removed for double-blind review]}.

\section*{Code Availability}

All analysis code and reproduction scripts are available at \url{[URL removed for double-blind review]}.

\section*{Preprint}

%% Preprint details removed for double-blind review. Disclosed in cover letter.
...
[Removed for double-blind review. See separate Declaration of Competing Interest file.]

\section*{Funding}

[Removed for double-blind review. See Title Page.]

%% ================================================================
%% REFERENCES
%% ================================================================
\bibliographystyle{elsarticle-harv}
\bibliography{eaai-references-blind}
