%%
%% Agentic AI Paper — engrXiv Preprint Format  (VERSION 2)
%% Paper #1 in the AI Systems Landscape 2026 series
%% License: CC BY 4.0
%%
%% v2 changes (May 2026):
%%   - Expanded PPAS vs. OODA architectural comparison (§3 / Comparison with Classical Models)
%%   - Added §8.4 Framework Validation: Layer Separability
%%
...
PPAS is architecturally distinct from each prior model. ReAct~\citep{yao2023react} collapses Plan and Act into a single interleaved step and lacks explicit self-correction. Reflexion~\citep{shinn2023reflexion} adds self-correction but does not separate planning from acting. The BDI model~\citep{rao1995bdi} separates belief updating from intention formation but lacks tool use and the iterative correction loop central to LLM agents.

OODA (Observe--Orient--Decide--Act; \citealt{boyd1987ooda}) warrants closer comparison because it too has four phases and a cyclical structure. PPAS differs from OODA in four architecturally consequential ways. First, \emph{Plan} and OODA's \emph{Orient} have distinct semantics: Orient updates a mental model in adversarial, time-critical contexts (originally fighter-pilot engagements with sub-second cycles), whereas PPAS's Plan generates goal-conditioned multi-step strategies over an explicit memory state $\mathcal{S}$. Second, PPAS's \emph{Self-Correct} phase is a formal feedback operator $\text{correct}(\mathcal{S}_{t-1}, a_t, o_t, g)$ that may trigger re-planning, memory consolidation, or human escalation based on observed action outcomes---none of which exist in OODA's linear decide-act sequence, designed for reactive tactical decisions rather than iterative goal-directed computation. Third, the PPAS action space is structurally bounded by the tool set $\mathcal{T}$ in the agent tuple $\mathcal{A} = \langle \mathcal{M}, \mathcal{T}, \mathcal{S}, \pi \rangle$; OODA has no analogous concept, as it was designed for embodied decision-making rather than computational agents that must discover and invoke heterogeneous APIs at runtime. Fourth, OODA offers no formal memory specification, whereas PPAS's state space $\mathcal{S}$ is an explicit, managed component enabling principled memory persistence across sessions. In aggregate, PPAS is not a renaming of OODA but a re-grounding of agent-loop design for the LLM era, where planning is generative and goal-conditioned, self-correction is a formal computational operator, tools are first-class architectural citizens, and memory is explicitly managed state.
...
Taken together, the three studies provide empirical validation of the 8-layer stack's core architectural claim: that the layers are \emph{separable}---independently variable without restructuring the remainder of the system. Study~2 holds five layers constant (L1: Claude 3.5 Sonnet; L2: LangGraph; L3: identical vector store; L4: identical tool set; L8: none) while varying only L6 (Planning \& Reflection design pattern), isolating a 17.7 percentage-point accuracy range on HotpotQA (61.2\% ReAct to 78.9\% HITL + Reflection). If the layer boundaries were merely taxonomic labels rather than genuine architectural seams, this controlled isolation of a single layer's effect would not be operationally achievable. Study~3 independently validates L5 (Inter-Agent Protocols) by holding all other layers constant across three communication configurations (MCP, A2A, direct), yielding measurable task-success and delegation-accuracy differences attributable solely to protocol choice at L5. Study~1 corroborates at the ecosystem scale: the 39.3 percentage-point SWE-Bench gap between HITL~+ Reflection agents (72.5\%) and ReAct agents (33.2\%) across independent organisations using comparable foundation models confirms that L6 choices have first-order outcome effects independent of L1. Collectively, these three studies demonstrate that the 8-layer stack is not a descriptive taxonomy but an engineering decomposition whose layer boundaries correspond to independently measurable, causally isolable performance dimensions.
